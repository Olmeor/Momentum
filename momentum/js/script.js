console.log (`'Score: 150 / 150 - нет дополнительного функционала (Todo).
...
+ текст приветствия меняется в зависимости от времени суток (утро, день, вечер, ночь) +5
  ...
  с 00:00 до 5:59 - Good night / Доброй ночи +
...
(WebP не понравился, весит на 10% больше правильно сжатого jpg, но качество при этому хуже, чем у jpg)
...
9. Получение фонового изображения от API +10
  Пункт считается выполненным, если фоновые изображения, полученные от API, отвечают требованиям к фоновым изображениям, указанным в пункте 3:
  + их можно перелистывать кликами по стрелкам, обеспечивается плавная смена фоновых изображений +
  + ссылка на фоновое изображение формируется с учётом времени суток + если пользователь не указал другие теги для их получения.
...
11. Дополнительный функционал на выбор - не реализовано

P.S. Дорогой друг! Если в консоль вываливаются ошибки, возможно проглючило локальное хранилище. Его необходимо почистить. Для этого необходимо в браузере зайти в инструменты разработчика (F12), в верхнем меню, там где Console, которую ты сейчас читаешь, выбрать Application, в этом окне выбрать Storage - Local Storage, нажать ПКМ на ссылку на эту страницу и выбрать Clear. Далее обновить страницу и все должно заработать.
`)